...
it by passing the `alignRight` prop.

<ReactPlayground codeText={MenuAlignRight} />

### Responsive alignment

If you want to use responsive alignment, use the `align` prop on the `<DropdownMenu>`, 
or the `menuAlign` prop on the `<DropdownButton>` and `<SplitButton>`. 
You can specify the directions `left` or `right` at each of the various breakpoints.

...
A Toast can also automatically hide after X milliseconds using the `autohide` prop with the `delay` prop to specify the delay. But be aware that autohide only triggers when the `onClose` function is called. To open the toast manually change the show property.
